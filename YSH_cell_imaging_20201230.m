function varargout = YSH_cell_imaging_20201230(varargin)
...
% Last Modified by GUIDE v2.5 24-Dec-2020 20:19:35
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @YSH_OpeningFcn, ...
                   'gui_OutputFcn',  @YSH_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function YSH_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = YSH_OutputFcn(hObject, eventdata, handles) 
...
% --- Upgrade YSH 20201226 for cell image TIF save
...
%% YSH_modification 20201228 aquire rotated image of a cell

rect = handles.myrect;
rect(1) = rect(1) + celldata(7) - 20;
rect(2) = rect(2) + celldata(8) - 20;
rect(3) = 40;
rect(4) = 40;

IF = imcrop(handles.fl(handles.flframe).data,rect);
IPC = imcrop(handles.ph(handles.phframe).data,rect);
IPC = imbinarize(IPC);
IPC = imcomplement(IPC);


if celldata(11) > celldata(12)
    yb = celldata(11);
    xa = celldata(12);
    angle = celldata(13) + pi()/2;
else
    xa = celldata(11);
    yb = celldata(12);
    angle = celldata(13);
...
%IF = handles.crop;
Irf = imrotate(IF,angle/pi()*180);
Irf = imcrop(Irf,[length(Irf(1,:))/2-20 length(Irf(:,1))/2-20 40 40]);

Irpc = imrotate(IPC,angle/pi()*180);
Irpc = imcrop(Irpc,[length(Irpc(1,:))/2-20 length(Irpc(:,1))/2-20 40 40]);

imwrite(IF,['selected' num2str(handles.flframe) '.tif'])
imwrite(Irf,['rotated_IF' num2str(handles.flframe) '.tif'])
imwrite(Irpc,['rotated_IPC' num2str(handles.flframe) '.tif'])
